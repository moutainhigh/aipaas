package com.ai.paas.ipaas.search.common;
...
public class TypeGetter<T> {
	...
	protected TypeGetter() {
		...
		this.rawType = (Class<? super T>) TypeHelper.getRawType(type);
	 ...
	 */
	@SuppressWarnings("unchecked")
	TypeGetter(Type type) {
		this.type = TypeHelper.canonicalize(TypeHelper.checkNotNull(type));
		this.rawType = (Class<? super T>) TypeHelper.getRawType(this.type);
	...
	static Type getSuperclassTypeParameter(Class<?> subclass) {
		...
		return TypeHelper.canonicalize(parameterized.getActualTypeArguments()[0]);
	}
	...
	public boolean isAssignableFrom(Type from) {
		...
		if (type instanceof Class<?>) {
			return rawType.isAssignableFrom(TypeHelper.getRawType(from));
	 ...
	 */
	@Deprecated
	public boolean isAssignableFrom(TypeGetter<?> token) {
	...
	private static boolean isAssignableFrom(Type from, ParameterizedType to, Map<String, Type> typeVarMap) {
		...
		Class<?> clazz = TypeHelper.getRawType(from);
	...
	public final boolean equals(Object o) {
		return o instanceof TypeGetter<?> && TypeHelper.equals(type, ((TypeGetter<?>) o).type);
	}

	@Override
	public final String toString() {
		return TypeHelper.typeToString(type);
	}

	/**
	 * Gets type literal for the given {@code Type} instance.
	 */
	public static TypeGetter<?> get(Type type) {
		return new TypeGetter<Object>(type);
	}

	/**
	 * Gets type literal for the given {@code Class} instance.
	 */
	public static <T> TypeGetter<T> get(Class<T> type) {
		return new TypeGetter<T>(type);
	}
	 ...
	 */
	public static TypeGetter<?> getParameterized(Type rawType, Type... typeArguments) {
		return new TypeGetter<Object>(TypeHelper.newParameterizedTypeWithOwner(null, rawType, typeArguments));
	}
	 ...
	 * {@code componentType}.
	 */
	public static TypeGetter<?> getArray(Type componentType) {
		return new TypeGetter<Object>(TypeHelper.arrayOf(componentType));
	}
}
